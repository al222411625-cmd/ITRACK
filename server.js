...
function countWarrantyExpiringSoon(activos, days = 30) {
  const now = new Date();
  const limit = new Date(now.getTime() + days * 24 * 60 * 60 * 1000);
  return activos.reduce((count, activo) => {
    const fecha = parseDateValue(activo.garantiaHasta);
    if (!fecha) return count;
    return fecha >= now && fecha <= limit ? count + 1 : count;
  }, 0);
}
...
app.get('/api/dashboard', requireAuth, async (req, res) => {
  try {
    ...
    const activosConGarantia = await Activo.find({ garantiaHasta: { $exists: true, $ne: null, $ne: '' } });
    const warrantyExpiringSoon = countWarrantyExpiringSoon(activosConGarantia, 30);
...
// ====================== GENERAR QR PARA ACTIVO ======================
app.get('/api/activos/:id/qr', requireAuth, async (req, res) => {
  try {
    const { id } = req.params;
