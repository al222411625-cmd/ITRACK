...
async function loadDashboardData() {
  ...
  <div class="dashboard-item rounded-4 p-3 text-center">
    <h6 class="text-info">Mantenimientos activos</h6>
    <h3 class="text-white">${data.activosMantenimiento || 0}</h3>
  </div>
  <div class="dashboard-item rounded-4 p-3 text-center">
    <h6 class="text-info">Garantías próximas</h6>
    <h3 class="text-white">${data.warrantyExpiringSoon || 0}</h3>
  </div>
`;
  ...
  ${data.activosMantenimiento > 0 || data.warrantyExpiringSoon > 0 ? `
  <div class="alert alert-warning mt-3" role="alert">
    ${data.activosMantenimiento > 0 ? `Hay ${data.activosMantenimiento} equipo(s) en mantenimiento activo. ` : ''}
    ${data.warrantyExpiringSoon > 0 ? `Hay ${data.warrantyExpiringSoon} garantía(s) próxima(s) a vencer en los próximos 30 días.` : ''}
  </div>
  ` : ''}
  <div class="card border-0 shadow-sm p-4 h-100">
  ...
  try {
    ...
    contentArea.innerHTML = `
      ...
      ${canAdd ? renderForm('Registrar nuevo activo', [
        ...
        { id: 'fechaCompra', label: 'Fecha de compra', type: 'date', placeholder: '', required: false },
        { id: 'garantiaHasta', label: 'Garantía hasta', type: 'date', placeholder: '', required: false },
        { id: 'proveedor', label: 'Proveedor', type: 'text', placeholder: 'Nombre del proveedor', required: false },
        { id: 'costo', label: 'Costo', type: 'number', placeholder: '', required: false }
      ...
      document.getElementById('formActivos').addEventListener('submit', async event => {
        ...
        try {
          const payload = {
            ...
            garantiaHasta: document.getElementById('garantiaHasta').value,
            proveedor: document.getElementById('proveedor').value,
            costo: parseFloat(document.getElementById('costo').value) || undefined
          };
      ...
      <div class="table-responsive bg-surface p-3 rounded-4 shadow-sm mb-4">
        <table class="table table-hover align-middle mb-0">
          ...
          <tbody>
            ...
            ${mantenimientos.length === 0 ? `<tr><td colspan="${canDelete ? 6 : 5}" class="text-center text-muted py-4">No hay mantenimientos programados.</td></tr>` : ''}
          </tbody>
        </table>
      </div>
      ${canAdd ? `
